...
template <class OperatorMap, class OperatorReduce, class IteratorRead, class IteratorWrite>
void Naive(IteratorRead aBegin, IteratorRead bBegin, IteratorWrite cBegin,int sizeN, int sizeK, int sizeM) {
...
inline void ReferenceImplementation(
    CONFIG_DTYPE const *a, CONFIG_DTYPE const *b, CONFIG_DTYPE *c,
    const unsigned size_n,
    const unsigned size_k,
    const unsigned size_m) {
    CallBLAS<CONFIG_DTYPE, OperatorMap, OperatorReduce>(a, b, c, size_n, size_k, size_m);
}

template <typename T>
typename std::enable_if<std::is_integral<T>::value, signed long>::type make_signed(T val) {
    return static_cast<signed long>(val);
}

template <typename T>
typename std::enable_if<std::is_floating_point<T>::value, T>::type make_signed(T val) {
    return val;
}

template <typename T>
typename std::enable_if<std::is_same<T, half>::value, half>::type make_signed(half val) {
    return val;
}
